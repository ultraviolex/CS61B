...
public class ArrayHeap<T> {
    ...
    private int min(int index1, int index2) {
        ...
        double lower = Math.min(getNode(index1)._priority, getNode(index2)._priority);
        if (lower == getNode(index1)._priority) {
            return index1;
        }
        return index2;
    }
    ...
    private void bubbleDown(int index) {
        ...
        double leftPriority = getNode(leftIndex)._priority;
        double rightPriority = getNode(rightIndex)._priority;
        if (getNode(index)._priority > leftPriority && getNode(index)._priority > rightPriority) {
            int swapIndex = min(leftIndex, rightIndex);
            swap(index, swapIndex);
            bubbleDown(swapIndex);
        } else if (getNode(index)._priority > leftPriority) {
            swap(index, leftIndex);
            bubbleDown(leftIndex);
        } else if (getNode(index)._priority > rightPriority) {
    ...
    public T removeMin() {
        ...
        if (size() == 1) {
            return removeNode(1)._item;
        }
        T smallestItem = removeNode(1)._item;
        swap(1, size());
        bubbleDown(1);
        return smallestItem;
    }
    ...
    public void changePriority(T item, double priority) {
        ...
        getNode(index)._priority = priority;
        bubbleDown(index);
        bubbleUp(index);
    }
}
