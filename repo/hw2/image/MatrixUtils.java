 ...
 *
 *  @author Josh Hug and Xiuhui Ming
 */
...
public class MatrixUtils {
    ...
    private static double get(double[][] e, int r, int c) {
        if (r == 0) {
            ...
            return Math.min(e[r - 1][c] + e[r][c], e[r - 1][c + 1] + e[r][c]);
        } else if (c == e[0].length - 1) {
            return Math.min(e[r - 1][c] + e[r][c], e[r - 1][c - 1] + e[r][c]);
        } else {
            return Math.min(Math.min(e[r - 1][c - 1] + e[r][c], e[r - 1][c] + e[r][c]),
                    e[r - 1][c + 1] + e[r][c]);
        }
    }
    ...
    public static double[][] accumulate(double[][] m, Orientation orientation) {
        double[][] mT = MatrixUtils.copy(m);
        if (orientation == Orientation.VERTICAL) {
            mT = accumulateVertical(m);
        } else {
            mT = transpose(m);
    ...
    private static double[][] transpose(double[][] m) {
        ...
        for (int x = 0; x < transposed.length; x++) {
            for (int y = 0; y < transposed[0].length; y++) {
