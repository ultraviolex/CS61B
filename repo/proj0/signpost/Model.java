...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int i = 1; i <= last; i++) {
            if (_solnNumToPlace[i] == null) {
                throw badArgs("Missing the number " + i);
            }
        }
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                _board[x][y]._successors =
                        _allSuccessors[x][y][_board[x][y]._dir];
            }
        }
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                for (Place pl : _board[x][y]._successors) {
                    get(pl)._predecessors.add(_board[x][y].pl);
                }
            }
        }
        _unconnected = last - 1;
    ...
    Model(Model model) {
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                Sq og = model.get(x, y);
                _board[x][y] = new Sq(x, y, og.sequenceNum(), og.hasFixedNum(),
                        og.direction(), og.group());
                _allSquares.add(_board[x][y]);
                _board[x][y]._predecessors = new PlaceList();
            }
        }
        for (Sq i : _allSquares) {
            Sq og = model.get(i.pl);
            i._successors = og._successors;
            i._predecessors = og._predecessors;
            if (og._successor != null) {
                i._successor = this.get(og.successor().pl);
            }
            if (og._predecessor != null) {
                i._predecessor = this.get(og.predecessor().pl);
            }
            if (og.head() != null) {
                i._head = this.get(og.head().pl);
            }
        }
    }
    ...
    boolean autoconnect() {
        ...
        for (Sq s : _allSquares) {
            if (s.sequenceNum() != 0 && s.successor() == null
                    && s.successors() != null) {
                PlaceList possibleConnect = s.successors();
                for (Place pl : possibleConnect) {
                    Sq possibleConnectSq = get(pl);
                    if (possibleConnectSq.sequenceNum()
                            == s.sequenceNum() + 1) {
                        s.connect(possibleConnectSq);
                        changed = true;
                    }
                }
            }
    ...
    private int arrowDirection(int x, int y) {
        ...
        for (int x0 = 0; x0 < _solution.length; x0++) {
            for (int y0 = 0; y0 < _solution[0].length; y0++) {
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            boolean correctDirection = (this.direction()
                    == Place.dirOf(this.x, this.y, s1.x, s1.y));
            boolean noPredSucc = (s1.predecessor() == null
                    && this.successor() == null && s1.sequenceNum() != 1
                    && this.sequenceNum() != _width * _height);
            boolean seqNum = true;
            if (this.sequenceNum() != 0
                    && s1.sequenceNum() != 0) {
                seqNum = (this.sequenceNum() == s1.sequenceNum() - 1);
            } else if (this.sequenceNum() == 0
                    && s1.sequenceNum() == 0) {
                seqNum = (this.group() != s1.group() || this.group() == -1);
            }
            return (correctDirection && noPredSucc && seqNum);
        }
        ...
        boolean connect(Sq s1) {
            ...
            s1._head = this._head;
            for (Sq i = s1; i != null; i = i._successor) {
                i._head = this._head;
            }
            if (this.sequenceNum() != 0 && s1.sequenceNum() == 0) {
                releaseGroup(s1._group);
                s1._group = this._group;
                for (Sq i = this; i._successor != null; i = i._successor) {
                    i._successor._sequenceNum = i._sequenceNum + 1;
                }
                for (Sq i = this; i != null; i = i._successor) {
                    i._group = this._group;
                }
            } else if (this.sequenceNum() == 0 && s1.sequenceNum() != 0) {
                releaseGroup(this._group);
                this._group = s1._group;
                for (Sq i = s1; i._predecessor != null; i = i._predecessor) {
                    i._predecessor._sequenceNum = i._sequenceNum - 1;
                }
                for (Sq i = s1; i != null; i = i._predecessor) {
                    i._group = s1._group;
                }
            }
            if (this.sequenceNum() == 0 && s1.sequenceNum() == 0) {
                int joinedGroup = joinGroups(this._group, s1._group);
                for (Sq i = s1; i != null; i = i._successor) {
                    i._group = joinedGroup;
                }
                for (Sq i = this; i != null; i = i._predecessor) {
                    i._group = joinedGroup;
                }
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (this.predecessor() == null && next.successor() == null) {
                    ...
                    next._head = next;
                    int group = newGroup();
                    setGroup(next, group, -1);
                }
            } else {
                boolean fixedSeqNumThis = false;
                boolean fixedSeqNumNext = false;
                for (Sq i = this; i != null; i = i.predecessor()) {
                    if (i.hasFixedNum()) {
                        fixedSeqNumThis = true;
                    }
                }
                if (!fixedSeqNumThis) {
                    int group = -1;
                ...
                for (Sq i = next; i != null; i = i.successor()) {
                    if (i.hasFixedNum()) {
                        fixedSeqNumNext = true;
                    }
                }
                if (!fixedSeqNumNext) {
                    int group = -1;
        ...
        /** Helper function to set groupNum.
         * @param s the Sq sequence
         * @param groupNum what to set group of each element in s to
         * @param dir -1 is predecessors and 1 and successors */

        private void setGroup(Sq s, int groupNum, int dir) {
            if (dir == 1) {
                for (Sq i = s; i != null; i = i._successor) {
                    i._group = groupNum;
                }
            } else {
                for (Sq i = s; i != null; i = i._predecessor) {
                    i._group = groupNum;
                }
            }
        }
